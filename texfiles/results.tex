...
In this chapter and the next one, after general results on \ccsp(\mrelset)
on monotone \mrelset, we use \cbis\ as a yardstick to clarify some \chom(\mH)
problems as ``not too hard'' and ``not too easy''\@.
We first show that for \emph{monotone} relations, there is an
