...
Constraint Satisfaction Problem (CSP) is the problem of deciding if it is possible to
...
if there is a function \(f\) satisfying all the given constraints. \ccsp\ is the
...
In this work we are going to study the complexity of finding approximate solutions for \ccsp\@.
Just a few number of problems in real world applications
are efficiently solvable; however, for many of them approximate
solution is satisfactory. 
In order to find out which problems can be solved efficiently, we classify them
by their complexity of approximation. We also mention some of
...
In this work domain and relations are always finite.
...
which is a tuple of variables from \(V\) and a relation \(R\) of the same arity as \(\varrho\)
from \mrelset\@. A constraint is represented as \(\const{\varrho,R}\)\@.
\end{itemize}
...
We continue with \tcoloring\ problem from Example~\ref{exm:3col}.
...
CSP for 3-coloring on \mG\ is as follows. The set of variables is the set of vertices of \mG,
and for each edge of \mG\ a constraint with relation \(NEQ_{rgb}\) is placed.
\begin{align*}
\mathcal{P}=(&D=\setof{1,2,3},V=\setof{v_1,v_2,v_3,v_4,v_5}, \\
&C=\{\const{(v_1,v_2),NEQ}, \const{(v_2,v_3),NEQ},\const{(v_3,v_4),NEQ},\\
&\const{(v_4,v_5),NEQ}, \const{(v_5,v_1),NEQ},
\const{(v_2,v_3),NEQ}\})
\end{align*}
\end{example}
\begin{figure}
...
\ccsp(\mrelset) is the problem of finding the number of assignments for a
...
a homomorphism from \mG\ to \mH\ is a mapping \(h\) from
...
Let \pname{NoRB-3-Coloring} be \pname{3-Coloring} problem with an additional
restriction that the vertices colored blue are not allowed to be connected 
to the vertices colored red. In order to formulate this problem with graph homomorphism,
...
the \pname{NORB-3-Coloring} problem.
This example shows that graph homomorphism generalizes graph coloring. This is why
...
defines the \emph{Lov\'{a}sz vector} which is used to characterize graphs.
...
they have the same Lovasz vector.  \todo{Check}
...
\[w(\sigma)=\prod_{uv\in E_G}w_H(\sigma(u)\sigma(v))\prod_{v\in V_G}w_H(\sigma(v))\]
...
Note that, the \pname{H-Partition} problem is a special case of weighted \ccsp(\mrelset)\@.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Sampling}
A \emph{Uniform Generation} problem is a problem that its goal is to find a
uniformly random solution for a given instance.
An \emph{Almost Uniform Generation} is a problem that 
its goal is to find a solution for a given instance such that
...
Intuitively, a self-reducible problem is a problem that solutions for an instance can be
...
predict the behaviour of a system in a given temperature. Usually there is a complex system
...
Let \(\beta\) be the inverse temperature. 
{\em Partition function} at a given temperature is defined as
\[Z=\sum_\sigma e^{-\beta H(\sigma)}\]
Partition function encodes the statical properties of the system in thermodynamic equilibrium.
...
&=\sum_{\sigma:V\to\setof{-1,+1}} \prod_{(u,v)\in E} e^{\beta J_{u,v}\sigma(u)\sigma(v)} \prod_{v\in V} 
e^{\beta l_v\sigma(v)}
\end{align*}
...
either for all \(v\), \(\mu_v\) is at least one or for all \(v\), \(\mu_v\) is at most one.
Jerrum and Sinclair \cite{Jer93} have proved that exact computation of problem in
the Ising model is hard to compute.

The next commonly studied model is the Ashkin-Teller model.
This model is a generalization of the Ising model to four-component systems.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Reductions and Complexity}
...
Two important sets of functions we are interested in are FP and \cp\@.
FP is the set of functions computable in polynomial time and \cp\ is
the set of function that can be expressed as the
number of accepting paths of a non-deterministic polynomial time running Turing machine. 
...
\begin{defi}[\cpC ness] 
A problem is \cpc\ if it is a member of \cp\ and every problem in \cp\ is 
Turing reducible to it in polynomial time.
\end{defi}
...
In the same way that Cook \cite{cook1971} proved SAT is NP-complete,
Valiant \cite{Valiant1979} proved \csat\ is \cpc\@. It is easy to see that 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Clones}
In instances of CSP, some relations are explicitly forced on a scope of variables by 
given constraints, while interactions of constraints can implicitly force other relations
on other scopes of variables.

\begin{example}
Let \(NEQ=\setof{(0,1),(1,0)}\) be a binary relation over the set
\(\setof{0,1}\)\@. Consider the following instance of 
CSP(\(NEQ\)) :
\[\mathcal{P}=(V=\setof{x,y,z}, D=\setof{0,1},
 C=\setof{\const{(x,z),NEQ},\const{(y,z),NEQ}}\]
...
\todo{Add entry to affine relations}
...
Alternatively, affine relations are the relations invariant under functions in the form
...
\begin{figure}[h]
%\centering 
\subfigure[\(Wr_0\)]{\input{figs/Wr0.pdftex}\label{fig:Wr0}}\hspace{4.78cm}
\subfigure[\(Wr_1\)]{\input{figs/Wr1.pdftex}\label{fig:Wr1}}\\
\subfigure[\(Wr_2\)]{\input{figs/Wr2.pdftex}\label{fig:Wr2}}\hfill
\subfigure[\(Wr_3\)]{\input{figs/Wr3.pdftex}\label{fig:Wr3}}
\caption{Wrench}
\label{fig:wrench}
\end{figure}
...
\begin{figure}[h]
\centering 
\subfigure[\(S^*_1\)]{\input{figs/wr1.pdftex}\label{fig:ss1}}\hfill
\subfigure[\(S^*_2\)]{\input{figs/wr2.pdftex}\label{fig:ss2}}\\
\subfigure[\(S^*_3\)]{\input{figs/wr3.pdftex}\label{fig:ss3}}\hfill
\subfigure[\(S^*_4\)]{\input{figs/wr4.pdftex}\label{fig:ss4}}
...
\begin{figure}[h]
\centering
\subfigure[\(\vec{P}^*_2\)]{\input{figs/ds.pdftex}\label{fig:ds}}\hfill 
\subfigure[\(P_4\)]{\input{figs/p4.pdftex}\label{fig:p4}}\hfill
\subfigure[Directed \(P_4\)] {\input{figs/dp4.pdftex}\label{fig:dp4}}\\
%\subfloat[$P^*_4$]{\input{figs/ps4.pdftex}\label{fig:ps4}}\hfill
\subfigure[\(\vec{P}^*_3\)]{\input{figs/dps3.pdftex}\label{fig:dps3}}\hfill
\subfigure[\(P^*_k\)]{\input{figs/psk.pdftex}\label{fig:psk}}
...
\(\land\) and \(\lor\) operators;\todo{and or} a Boolean constraint languages \mrelset\ is monotone if 
