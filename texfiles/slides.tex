...
Let \(H\) be a graph\\
If \(H\) is \textcolor{mygreen}{bipartite} or has a \textcolor{mygreen}{loop}, Hom(\(H\)) is polynomial time solvable\\
...
Without loss of generality assume \(\hom(Z,H_i) > \hom(Z,H_j)\)
\end{frame}
...
\GraphInit[vstyle=Shade]
\SetVertexMath
\node[inner sep=0,anchor=east,text width=2cm] (note1) at (1.7,-.5) {\(H_1\)};
\SetVertexNoLabel
\tikzset{VertexStyle/.style ={shape=circle, ball color = mygreen}}
...
\GraphInit[vstyle=Shade]
\SetVertexMath
\node[inner sep=0,anchor=east,text width=2cm] (note1) at (1.7,-0.5) {\(H_2\)};
\SetVertexNoLabel
\tikzset{VertexStyle/.style ={shape=circle, ball color = mygreen}}
...
\Vertex[Node]{F}
\Vertex[Node]{H}
\tikzset{EdgeStyle/.style ={thick,color = myred,}}
\Edges(B,F,H,D,B)
}
\end{tikzpicture}
\end{center}
...
\pause
\begin{block}{}
If \(G_1\) and \(G_2\) are monotone graphs, \(G_1\times G_2\) is a monotone graph
\end{block}
\end{frame}
...
\pause
\begin{lemma}
Let \(G\) be a monotone graph\\
Any retract \(H\) of \(G\) is also a monotone graph
\end{lemma}
\end{frame}
...
\pause
\item Let \(G'=G_1\times G_2 \times \dotsb G_k\)
\pause
\item Let \(G^r\) be a retract of \(G'\)
\end{itemize}
\pause
