...
\caption{Interval graph}\label{fig:intgi}
\end{figure}
...
\caption{Proper interval graph}\label{fig:pintgi}
\end{figure}
...
\caption{Graphs distinguishing (proper) interval graphs from monoton graphs}
\end{figure}
\end{example}
...
Lemma~\ref{lem:prod} shows that we can generate monotone relations from product of monotone relations.
...
Let \(G_r\) be the retraction of \(G'\) by \(r\)\@.
It would be nice if by proper choices of \(G_i\)s, \(G_r\)
was \mG\@. Unfortunately, this is not the case.
...
\mG\ in Figure~\ref{fig:erm3} is also a monotone graph. If we apply the previous process,
we can not generate \mG\@.
...
\caption{Graphs \ensuremath{G'} \ensuremath{G_r} \ensuremath{G}}
\end{figure}