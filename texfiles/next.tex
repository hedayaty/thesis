...
In this part, we indicate existence of an edge between vertices \(u\) and \(v\) as \(u \sim v\)\@.
...
\begin{figure}[h]
\hspace{2cm}
\subfigure[\ensuremath{G_3}]{\input{figs/ps4.pdftex}\label{fig:pintg}}\hspace{3cm}
...
Suppose that \(G_1\) is monotone. Suppose \(\top = a\), then
...
\(H_1\) is not an interval graph as well since the vertices \(e\), \(f\), \(g\), and \(g\) induce a subgraph isomorphic to the graph \(G_2\);
however, we show that \(H_1\) is a monotone graph in Example~\ref{exm:prod}.
is a proper interval graph. TODO continue
...
(proper) interval graph. However, in the Example~\ref{exm:diff} \(G_1\)
is not a monotone graph and it is an induced subgraph of \(H_2\); while, \(H_2\) is a monotone graph.
...
TODO say \(H_2\) is a proper interval graph and hence monotone and hence \(H_1\) is monotone!
\begin{figure}
\input{figs/ps3.pdftex}
\caption{Graph \ensuremath{H_2}}
\end{figure}
\end{example}
...
Let \(G_i\) be a proper interval graph derived by projection of \(G\) over \(C_i\)\@. \todo{This part needs more clearing}
