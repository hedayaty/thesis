...
proof for the problem of finding the number of truth assignment of a CNF\@.
Let \(C\) be a CNF\@. For a variable \(x\) used in \(C\), let \(C_x\) denote 
the CNF derived from \(C\) by removing all clauses having the literal \(x\) and also 
removing literal \(\bar x\) from other clauses. Analogously, let \(C_{\bar x}\) denote
the CNF derived from \(C\) by removing all clauses having the literal \(\bar x\) and also 
removing literal \(x\) from other clauses. Clearly, the number of solutions for
\(C\) is sum of number of solutions for \(C_x\) and \(C_{\bar x}\)\@.

In order to generate an almost uniform solution \(C\) using an approximate solver,
we first approximately find the number of solutions for \(C_x\) and \(C_{\bar x}\)\@.
Next, we assign \(x\) to true or false with probability proportional to number of solutions 
for \(C\) and \(C_{\bar x}\)\@. Then, we continue
with \(C\) or \(C_{\bar x}\) regarding the choice in previous step.

In order to solve approximate counting using an Almost Uniform Generator, first
we generate some number of random solutions. We evaluate the number of solutions
where \(x=true\) and the number of solutions where \(x=false\)\@. Without loss of generality
suppose the number of solutions where \(x=true\) is more. We approximately find the number of 
solutions for \(C_x\) and from the ratio we get from the random solutions, we estimate 
the number of solutions for \(C\)\@.
