...
\begin{theorem} [Hell and Ne\v{s}et\v{r}il 1990~\cite{pavol}] \label{thrm:pavol}
...
A \emph{uniform generation} problem is the problem of for an instance \(x\),
generating a uniformly random solution \(y\) such that \((x,y) \in R\)\@.
An \emph{almost uniform generation} is the problem of for an instance \(x\),
generating a random solution \(y\) 
...
In this model a \emph{counting} problem is the problem of for an instance \(x\),
finding the number of \(y\)s such that \((x,y)\in R\) and an approximate counting
problem is the problem of for an instance \(x\), approximately 
finding the number of \(y\)s such that \((x,y)\in R\)\@.
...
Many of the relation such as the relation between a graph and a matching  in the graph
and relation between CNF(Conjunctive Normal Form) or DNF(Disjunctive Normal Form) 
a satisfying truth assignment are self-reducible.

\begin{theorem} [Jerrum et al. 1986 \cite{JVV}] \label{thm:selfred}
For any self-reducible relation \mR, the almost uniform generation problem and the approximate counting
...
\emph{Sampling} is an extension of uniform generation; given a
...
and \(\log(|x|)\), respectively. There are no results on \ccsp(\mrelset) with
...
It is easy to see that \(\#\probi'=\#\probi^k\); \(\sqrt[k]{\apxalg(\probi')}\) is 
...
\item \(\apxalg\) runs in \(poly(|x|,\oneoeps)\) time.
\end{itemize}
\end{defi}
...
is an induced sub-graph of \mH\ and the problem \chom(\mH) is in FPRAS\@.
In other words, although \mG\ is an induced sub-graph of \mH, approximating
the problem \chom(\mH)
...
Since the \pname{\#Dual-SAT} problem is self-reducible, Theorem~\ref{thm:selfred} implies
that Algorithm~\ref{alg:dnf} can be used to find an approximate solution for\pname{Dual-SAT}\@.
...
Let \(Wr_q\) be the graph with vertex set \(V_q=\{a,b,c_1,c_2,\cdots,c_q\}\)
