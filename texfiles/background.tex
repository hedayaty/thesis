...
and \(w(e)=0\) for all \(e\not\in H\)\@. 
The problem \pname{H-Partition} is defined follows.
...
\begin{defi}[pp-definition]\label{def:ppdef}
...
which maintain {\em antitony}, that is, for every \(X,X'\subseteq A\) and every \(Y,Y'\subseteq B\) we have
\begin{align*}
X \subseteq X' \implies \fA(X) \supseteq \fA(X') \\
Y \subseteq Y' \implies \gB(Y) \supseteq \gB(Y') \\
\end{align*}
and {\em extensivity}, that is, for every \(X\subseteq A\) and every \(Y\subseteq B\) we have
\begin{align*}
X \subseteq \gB(\fA(X)) \\
Y \subseteq \fA(\gB(Y))
\end{align*}
...
the complexity of the problems CSP(\mrelset) and \ccsp(\mrelset) depends on \(Pol(\relset)\)\@.
However, it seems unlikely that a similar relation occurs for approximating \ccsp(\mrelset);
in fact, pp-definitions with existential quantifiers allowed, as in Definition~\ref{def:ppdef},
do not guarantee a reduction suitable for approximation.
