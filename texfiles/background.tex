...
are interreducible. \todo{Explain From 182-183 JVV}
...
can also be approximated by a randomized algorithm in polynomial time. \todo{Expand}
...
Note that SAT and \csat\ are generalizations of \tsat\ and \ctsat\ problems
mentioned in Examples~\ref{example:tsat,example:c3sat}\@. \todo{Fix reference after fixing original}
...
Functions \(Inv\) and \(Pol\) maintain\todo{Could not read the comment}
...
\todo{Explain how to express these conditions as polymorphisims}
...
and \(\log(|x|)\), respectively. There has been no results on \ccsp(\mrelset) with
