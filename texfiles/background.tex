...
The problem is usually reduced to a sampling problem and 
the Markov Chain Monte Carlo method solves the sampling problem as follows.
Consider a Markov chain with state space \(\Omega\) and stationary distribution \(\pi\)\@.
Informally, a Markov chain is said to be \emph{ergodic} if regardless of the initial state,
the probability distribution over \(\Omega\) asymptotically converges to \(\pi\)\@.
Start sampling from an arbitrary state in \(\Omega\); simulate the
Markov chain for \(T\) steps; finally output the final result.

We need to choose \(T\) such that after \(T\) steps, the distribution of state
is arbitrary close to the desired distribution \(\pi\)\@. The number of steps required
for the algorithm to become close enough to \(\pi\) is called the \emph{mixing time}\@.
Loosely, if the mixing time is polynomial in terms of the size of the input, then
the Markov chain is \emph{rapidly mixing}\@.
...
\(\bG_0=G\) and \(\bG_{i+1}\) is obtained by removing an arbitrary edge from \(\bG_i\)\@. 
The last element \(\bG_m\) is an empty graph which is k-colorable in \(k^n\) ways.
Next, we estimate the \(Z(G_i)/Z(G_{i+1})\) by sampling\@.
Consider a Marov chain whose state space \(\Omega_k\) is the set of all k-colorings
of the graph \(G_{i+1}\)\@. The transition probabilities from the state \(X_t\) are modeled as 
\begin{inparaenum}[(a)]
...
The algorithms chooses a sufficiently long random walk 
...
\todo{Add proofs between \cbis\ and \cds}
